package com.aeharake.choters.mocker
...
class UsersGenerator {
    companion object {
        ...
        fun getRandomFullNames(): List<FullName> {
            val shuffledFirstNames = shuffleFirstNames()
            val shuffledLastNames = shuffleLastNames()
            ...
            return mutableListOf<FullName>().apply {
                var i = 0
                while (i < 200) {
                    add(FullName(shuffledFirstNames[i], shuffledLastNames[i]))
                    i++;
                }
            }
        }
